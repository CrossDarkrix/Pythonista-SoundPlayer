...
cmdcenter = MPRemoteCommandCenter.sharedCommandCenter()
...
def main():
    ...
    ListView(soundP)
    SeekBarThread = SeekBar(soundP)
    concurrent.futures.ThreadPoolExecutor().submit(SeekBarThread.start)
    cmdcenter.playCommand().removeTarget_(soundP)
